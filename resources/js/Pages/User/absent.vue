            ...
            <Link :href="route('dashboard.leave')" class="col-xs-12 col-sm-3 custom-link">
               ...
               <div class="col-xs-12">
                   <div class="q-pa-sm row">
                       <div class="text-lg text-bold text-dark">Absent on Today's</div>
                       <q-space></q-space>
                   ...
                   <q-list>
                        <div v-for="item in absent_users.data" :key="item.id" class="flex justify-between items-center shadow-default q-mt-sm bg-white q-pa-sm">
                            <div>
                        ...
                        <div class="flex q-gutter-sm">
                            <q-btn color="primary" @click="navigate(absent_users.prev_page_url)" :disable="!Boolean(absent_users.prev_page_url)" round flat icon="o_chevron_left"/>
                            <q-btn color="primary" @click="navigate(absent_users.next_page_url)" :disable="!Boolean(absent_users.next_page_url)" round flat icon="o_chevron_right"/>
                        </div>
                   </q-list>
               </div>
           </div>
...
const props = defineProps(['user', 'devices', 'attendances', 'current_sessions', 'total_users', 'absent_users','noOfLeave','count_attendances','search'])
const q = useQuasar();
...
const navigate = async url => {
    if (url) {
        ...
        props.absent_users = response.props.absent_users;
    }
}
...
const handleSearch=e=>{
    router.get(route('dashboard.absent'), {
        search: state.search
    });
