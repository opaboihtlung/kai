<template>
    <q-page class="container" padding>
        ...
        <q-list separator class="bg-white shadow-default ">
            ...
            <q-chip>Date: {{ item?.start_date }}</q-chip>
                <q-item-section side>
                ...
                <q-btn-dropdown class="q-pa-xs" dropdown-icon="more_vert" right>
                        <q-list>
                            ...
                            <q-item v-if="item.status === 'Submitted' || item.status === 'Rejected'" v-close-popup clickable @click.stop="leftEarlyApprove(item, 'approve')">
                                <q-item-section side>
                            ...
                            <q-item v-if="item.status === 'Submitted' || item.status === 'Rejected'" v-close-popup clickable @click.stop="leftEarlyReject(item, 'reject')">
                                <q-item-section side>
...
const leftEarlyApprove=item=>{
    ...
    .onOk(()=>{
        router.post(route('appeal.approveleft2',item.id),{},{
...
const leftEarlyReject=item=>{
    q.dialog({
        ...
        .onOk(()=>{
            router.post(route('appeal.rejectleft2',item.id),{},{
                onStart:params => q.loading.show(),
                onFinish:params => q.loading.hide()
            })
        })
...
const handleSearch=e=>{
    router.get(route('appeal.left_early'), {
        search: state.search
    });
