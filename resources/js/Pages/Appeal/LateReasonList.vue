<template>
    <q-page class="container" padding>
        ...
        <q-list separator class="bg-white shadow-default ">
            ...
            <q-item-section>{{ item?.reason }}</q-item-section>
            <!-- <q-item-section>Date: {{ item?.start_date }}</q-item-section> -->

            <q-chip square><q-item-section>Date: ({{ item?.start_date }})
                ...
                <q-btn-dropdown class="q-pa-xs" dropdown-icon="more_vert" right>
                        <q-list>
                            ...
                            <q-item v-if="item.status === 'Submitted' || item.status === 'Rejected'" v-close-popup clickable @click.stop="lateReasonApprove(item, 'approve')">
                                <q-item-section side>
                            ...
                            <q-item v-if="item.status === 'Submitted' || item.status === 'Rejected'" v-close-popup clickable @click.stop="lateReasonReject(item, 'reject')">
                                <q-item-section side>
...
const lateReasonApprove=item=>{
    ...
    .onOk(()=>{
        router.post(route('appeal.approvelate2',item.id),{},{
...
const lateReasonReject=item=>{
    q.dialog({
        ...
        .onOk(()=>{
            router.post(route('appeal.rejectlate2',item.id),{},{
                onStart:params => q.loading.show(),
                onFinish:params => q.loading.hide()
            })
        })
...
const handleSearch=e=>{
    router.get(route('appeal.late_reason'), {
        search: state.search
    });
